...
1. Supervised learning does work and train on the generated dataset, but self supervised learning is challenging to apply with low linear probing accuracy. Cropping is required for good performance in the imagenet datasets. However, cropping severely decreases the performance for this synthetic dataset consisting of emojis and random backgrounds. This suggests the reason why cropping or SSL works on imagenet dataset is because each image patch is co-occuring or contains information about the other patches. So the challenge now is how do we naturally extract semantic structure in this synthetic dataset via SSL without manually using techniques like object detection because we want structure to naturally arise from data.
