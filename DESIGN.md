# SynthSSL — Dataset Generator

Synthetic dataset where every factor of variation is controlled at generation time and emitted as metadata, so any frozen representation can be probed on each factor via linear classification, KNN, or regression.

SSL training and the probing machinery live in `SSL/train.py` / `stable_pretraining`. This document describes the **generator and the labeled factors it exposes** — no training or augmentation logic here.
